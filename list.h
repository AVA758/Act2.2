...
// TO DO #1
// 
// Returns the value before the first occurrence of certain value.
//
...
T DoubleLinkedList<T>::after(T val) const {
	...
	p=head;
	for (int i=0; i < size; i++){
		if(p->value == val && i==(size-1)){
			throw NoSuchElement();
		}
		else if (p->value == val && i==(size-2)){
	    return last();
		}
		else if (p->value == val){
			q = p->next;
